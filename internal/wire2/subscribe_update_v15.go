...
func (m *SubscribeUpdate) append_v15(buf []byte) []byte {
	buf = quicvarint.Append(buf, uint64(m.RequestID))
	buf = quicvarint.Append(buf, uint64(m.SubscriptionRequestID))
...
func (m *SubscribeUpdate) parse_v15(data []byte) error {
	var err error
	var n int
